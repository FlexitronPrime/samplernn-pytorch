...
class SampleRNN(torch.nn.Module):
    ...
    def __init__(self, frame_sizes, n_rnn, dim, learn_h0, q_levels,
        ...
        self.frame_level_rnns = torch.nn.ModuleList([
            FrameLevelRNN(
                frame_size, n_frame_samples, n_rnn, dim, learn_h0, weight_norm, rnn_type
            )
...
class FrameLevelRNN(torch.nn.Module):
    ...
    def forward(self, prev_samples, upper_tier_conditioning, hidden,cell):
        ...
        if(self.rnn_type == 'LSTM'):
            if cell is None:
                cell = self.h0.unsqueeze(1) \
                            .expand(n_rnn, batch_size, self.dim) \
                            .contiguous()
            (output, res) = self.rnn(input, (hidden,cell))
            hidden= res[0]
            cell = res[1]        
...
class Runner:
    ...
    def reset_current_states(self):
        self.current_states = {rnn: None for rnn in self.model.frame_level_rnns}

    def run_rnn(self, rnn, prev_samples, upper_tier_conditioning, rnn_type):
        if(self.model.rnn_type =='GRU'):
            (output, new_hidden) = rnn(
                prev_samples, upper_tier_conditioning, self.hidden_states[rnn], None
            )
            self.hidden_states[rnn] = new_hidden.detach()
        if(self.model.rnn_type=='LSTM'):
            (output, new_hidden, new_current) = rnn(
                prev_samples, upper_tier_conditioning, self.hidden_states[rnn],self.current_states[rnn]
            )
            self.hidden_states[rnn] = new_hidden.detach()
            self.current_states[rnn] = new_current.detach()
        return output
...
class Predictor(Runner, torch.nn.Module):
    ...
    def forward(self, input_sequences, reset):
        ...
        for rnn in reversed(self.model.frame_level_rnns):
            ...
            #print(from_index, to_index ,input_sequences[:, from_index : to_index].size(),prev_samples.size())
            prev_samples = prev_samples.contiguous().view(batch_size, -1, rnn.n_frame_samples)

            upper_tier_conditioning = self.run_rnn(rnn, prev_samples, upper_tier_conditioning, self.model.rnn_type)
            ...
            with torch.no_grad():
                for (tier_index, rnn) in \
                    ...
                    frame_level_outputs[tier_index] = self.run_rnn(
                        rnn, prev_samples, upper_tier_conditioning, self.model.rnn_type
                    )
